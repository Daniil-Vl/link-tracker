...
public class TrackCommandTest extends AbstractCommandTest {
    ...
    void givenValidMessage_whenHandle_thenReturnSuccessMessage() {
        ...
        Mockito.when(mockedMessage.text()).thenReturn("/track %s".formatted(url.toString()));
        LinkResponse linkResponse = new LinkResponse(
            2L,
            url
        );
        String successMessage = "New resource added: url";
        Mockito.when(scrapperClient.addLink(chatId, url)).thenReturn(linkResponse);
    ...
    void givenMessageWithoutArgument_whenHandle_thenReturnHelpMessage() {
        Mockito.when(mockedMessage.text()).thenReturn("/track");
